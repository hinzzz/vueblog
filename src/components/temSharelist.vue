            ...
            showSearchShowList:function(initpage){//展示数据
                ...
                ShowArticleAll(that.artId,that.sendId,that.keywords,that.level,(result)=>{
                    ...
                    if(result && result.code==0){
                        if(result.page.records.length>0&&result.page.records.length<10){
                            that.hasMore = false
                        }else{
                            that.hasMore = true;
                        }
                        that.articleList = initpage ? result.page.records : that.articleList.concat(result.page.records);
                        //that.artId = result.page.records[result.page.records.length-1].id;
